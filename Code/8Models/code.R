...
for (iter in 1:(niter + nburn)) {
  ...
  if(update_term1){
    ...
    if(delta1 == 1){ # unconstrained model
      ...
      ax <- updateAX(ax, d, E, term234)  
      term1 <- matrix(ax, X, Y, byrow = F)
      
    } else if(delta1 == 2){ # linear model
      
      list_ab <- updateABX(ab, c2x, d, E, term234)
      ab <- list_ab$ab
  ...
  if(update_term3){
    ...
    if(delta3 == 2){ # varying line 
      ...
      list_bxk2t <- update_bxk2t_twosteps(bx, k2t, d, E, term124, delta2)
  ...
  if(update_delta3){
    ...
    if(delta3 == 1) { # no effect
      ...
      {
        list_proposal_kt <- findProposalk2t_given_bx(k2t, c2x, d, E, cxt)
        kt_star <- list_proposal_kt$bxl2t_star
        Sigma_kt_star <- list_proposal_kt$Sigma_star
        
        if(delta2 == 1){
          list_proposal_bx <- findProposalBX_given_k2t_m1(c2x, k2t, d, E, cxt, c2x)  
        } else if(delta2 == 2){
          list_proposal_bx <- findProposalBX_given_k2t_m2(c2x, k2t, d, E, cxt, c2x)  
        }
    ...
    if(delta3_proposed == 1){
      ...
      # find proposal for lee carter in two steps
      {
        ...
        if (delta2 == 1){
          ...
          list_proposal_bx <- findProposalBX_given_k2t_m1(c2x, k2t_proposed, 
                                                       d, E, cxt, c2x)
          
        } else  if (delta2 == 2){
          
          list_proposal_bx <- findProposalBX_given_k2t_m2(c2x, k2t_proposed, 
                                                       d, E, cxt, c2x)
